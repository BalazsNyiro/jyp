...
func Test_string_detection_simple(t *testing.T) {
	...
	elems_with_runes := elemRunesFromStr(`"name of king"`)
	elems_strings_detected := JsonCollectStringsInElems__removeSpaces(elems_with_runes)
...
func Test_string_detection_double(t *testing.T) {
	elems_with_runes := elemRunesFromStr(`"name": "Bob", "age": 7`)
	elems_strings_detected := JsonCollectStringsInElems__removeSpaces(elems_with_runes)
	wanted := Elem_list{
		ElemStr("name"),
		elemRune(':'),
...
func Test_string_detection_escaped_char(t *testing.T) {
	elems_with_runes := elemRunesFromStr(`"he is \"Eduard\""`)
	elems_strings_detected := JsonCollectStringsInElems__removeSpaces(elems_with_runes)
...
func Test_number_int_detection(t *testing.T) {
	elems_with_runes := elemRunesFromStr(`"price": 7.6, "age": 5`)
	elems_strings_detected := JsonCollectStringsInElems__removeSpaces(elems_with_runes)
	elems_num_detected := JsonCollectNumbersInElems(elems_strings_detected)
	ElemsPrint(elems_num_detected, 0)
	wanted := Elem_list{
		ElemStr("price"),
		elemRune(':'),
...
func Test_scalar_detection(t *testing.T) {
	elems := elemRunesFromStr(`"True": true, "False": false, "age": null `)
	elems = JsonCollectStringsInElems__removeSpaces(elems)
	elems = JsonCollectScalarsInElems(elems)
	// ElemsPrint(elems, 0)
	wanted := Elem_list{
		ElemStr("True"),
		elemRune(':'),
...
func Test_array_detection(t *testing.T) {
	elems := elemRunesFromStr(`"name": "Bob", "scores": [4, 6], "friends": [["Eve", 16], ["Joe", 42]], "key": "val"`)
	elems = JsonCollectStringsInElems__removeSpaces(elems)
	elems = JsonCollectNumbersInElems(elems)
	array := JsonCollectArraysInElems(elems)
...
func Test_json_render_object_pretty_print(t *testing.T) {
	...
	wanted := "{\n  \"human\": {\n    \"age\": 42,\n    \"name\": \"Bob\"\n  }\n}"
	rendered := root.jsonRenderPretty()
	if rendered != wanted {
...
func Test_object_detection(t *testing.T) {
	elem_root, _ := JsonParseSrc(`{"personal":{"city":"Paris", "cell": 123}}`)
	fmt.Println("Test_object_detection")
	ElemPrintOne(elem_root)
...
func Test_json_1(t *testing.T) {
	...
	elem_root, _ := JsonParseSrc(`{"name": "Bob", 
									 	"friends": [ 
                                     		{	"name":"Eve", 
												"cell": 123, 
												"age": 21
...
func Test_complex_big(t *testing.T) {
	elem_root, _ := JsonParseSrc(`{"name": "Bob", "friends": [ {"name":"Eve", "scores":[1,2]}, {"name":"Joe", "scores":[3,4]} ]}`)
...
func compare_receivedElems_wantedElems(receiveds Elem_list, wanteds Elem_list, t *testing.T) {
	...
	if len(receiveds) != len(wanteds) {
		...
		ElemsPrint(receiveds, 0)
		fmt.Println("    .......................    ")
		ElemsPrint(wanteds, 0)
		fmt.Println("<<< === compare, len !=   ===========")
...
func _compare_two_objects(objA Elem, objB Elem, t *testing.T) {
	for keyReceived, _ := range objA.ValObject {
		if ObjHasKey(objB.ValObject, keyReceived) == false {
